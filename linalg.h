...
// Compares vectors
inline bool vec3comp(vec3 a, vec3 b){
    return a[0] == b[0] && a[1] == b[1] && a[2] == b[2];
}

// Vector dumps

inline void vec3dump(vec3 v, FILE* fp = stdout)
{
    //fprintf(fp, "\n");
    fprintf(fp, "%g %g %g \n", v[0], v[1], v[2]);
}

inline void fvec3dump(fvec3 v, FILE* fp = stdout)
{
    fprintf(fp, "\n");
    fprintf(fp, "%g %g %g \n", v[0], v[1], v[2]);
}
...
inline void mat3dump(mat3 a, FILE* fp = stdout)
{
  fprintf(fp, "\n");
...
inline void  vec3copy( const vec3 a,  vec3 b) { memcpy(b, a, sizeof( vec3)); }
...
inline void mat3setRowWise(mat3 a, double data[9])
{
	a[0][0] = data[0]; a[0][1] = data[1]; a[0][2] = data[2];
	a[1][0] = data[3]; a[1][1] = data[4]; a[1][2] = data[5];
	a[2][0] = data[6]; a[2][1] = data[7]; a[2][2] = data[8]; 
}
